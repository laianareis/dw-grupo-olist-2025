...
def setup_environment():
    ...
    if not os.path.exists(db_path):
        db_path = DB_FILENAME 
        if not os.path.exists(db_path):
            files = [f for f in os.listdir(base_path) if f.endswith('.duckdb') or f.endswith('.db')]
            if files:
                db_path = os.path.join(base_path, files[0])
                print(f"[AVISO] Usando banco encontrado: {files[0]}")
            else:
                print(f"[ERRO] Banco de dados não encontrado em {base_path}")
                sys.exit(1)
...
def plot_freight_ratio(con):
    ...
    sns.barplot(x='ratio_frete', y='customer_state', data=df, hue='customer_state', 
...
def plot_dashboard(con, df_line, df_bar):
    ...
    # CORREÇÃO AQUI: Substituído 'USING SAMPLE' por 'ORDER BY random() LIMIT'
...
def main():
    ...
    try:
        ...
        con.close()
        print(f"\n--- Concluído. Imagens e Dashboards salvos em: {os.path.abspath(OUTPUT_DIR)} ---")
